...
<head>
    <meta charset="UTF-8">
    <title>Inventario Urbano - SIGEM</title>
    ...
    <style>
        ...
        .titulo-cuadro { font-size: 16px; font-weight: bold; color: #006633; margin-top: 20px; text-align: center; cursor: pointer; text-decoration: none; }
        ...
        .table-sigem { font-size: 13px; border-collapse: collapse; width: max-content; min-width: 100%; margin-bottom: 20px; }
        ...
        .table-sigem th, .table-sigem td { border: 1px solid #ccc; padding: 6px 10px; text-align: center; white-space: normal; font-size: 13px; max-width: 300px; word-break: break-word; }
                ...
                <select id="temaSelect" class="form-select w-75 mx-auto">
                    ...
                    if ($temas && mysqli_num_rows($temas) > 0) {
                        while ($fila = mysqli_fetch_assoc($temas)) {
                            $selected = ($fila['nombre_archivo'] === 'inventariourbano.php') ? 'selected' : '';
