...
@section('content')
                        ...
                        @if(isset($ce_contenidos) && count($ce_contenidos) > 0)
                            <div class="table-responsive">
                                <table id="tablaContenidosCE" class="table table-striped table-hover table-sm">
                                    ...
                                    <tbody>
                                        @foreach($ce_contenidos as $cont)
                                        <tr>
                                            <td><span class="badge bg-secondary">{{ $cont->ce_contenido_id }}</span></td>
                                            <td>
                                                <div style="max-width: 200px;">
                                                    <strong>{{ $cont->titulo_tabla ?: 'Sin título' }}</strong>
                                                    @if($cont->pie_tabla)
                                                        <br><small class="text-muted">{{ Str::limit($cont->pie_tabla, 60) }}</small>
                                                    @endif
                                                </div>
                                            </td>
                                            <td data-order="{{ $cont->tabla_filas * $cont->tabla_columnas }}">
                                                <span class="badge bg-info">
                                                    {{ $cont->tabla_filas }}×{{ $cont->tabla_columnas }}
                                                </span>
                                                <br><small class="text-muted">{{ $cont->tabla_filas * $cont->tabla_columnas }} celdas</small>
                                            </td>
                                            <td>
                                                @if($cont->subtema && $cont->subtema->tema)
                                                    <span class="badge bg-primary">{{ $cont->subtema->tema->tema }}</span>
                                                @else
                                            ...
                                            <td>
                                                @if($cont->subtema)
                                                    <span class="badge bg-success">{{ $cont->subtema->ce_subtema }}</span>
                                                @else
                                                    <span class="text-danger">Sin subtema</span>
                                                @endif
                                            </td>
                                            <td data-order="{{ $cont->created_at ? $cont->created_at->timestamp : 0 }}">
                                                @if($cont->created_at)
                                                    <small>{{ $cont->created_at->format('d/m/Y H:i') }}</small>
                                                @else
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-sm">
                                                    <button type="button" class="btn btn-outline-info" title="Ver Tabla" 
                                                            onclick="verTablaContenidoCE({{ $cont->ce_contenido_id }})">
                                                        <i class="bi bi-table"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-outline-warning" title="Editar" 
                                                            onclick="editarContenidoCE({{ $cont->ce_contenido_id }})">
                                                        <i class="bi bi-pencil"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-outline-danger" title="Eliminar" 
                                                            onclick="eliminarContenidoCE({{ $cont->ce_contenido_id }})">
                ...
                <div class="modal-content">
                    ...
                    <form id="formAgregarContenido" method="POST" action="{{ route('sigem.admin.consultas.contenido.crear') }}">
                        ...
                        <div class="modal-body">
                            ...
                            <!-- Contenido para la tabla editable -->
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Inicializar DataTables para las tres tablas
            initializeDataTable('#tablaTemasCE', {
                columnDefs: [
                    { targets: 0, width: "10%", className: "text-center" },
                    { targets: 1, width: "50%" },
                    { targets: 2, width: "20%", className: "text-center", type: "num" },
                    { targets: 3, width: "20%", className: "text-center", orderable: false, searchable: false }
                ],
                order: [[1, 'asc']],
                pageLength: 10,
            ...
            initializeDataTable('#tablaSubtemasCE', {
                columnDefs: [
                    { targets: 0, width: "8%", className: "text-center" },
                    { targets: 1, width: "35%" },
                    { targets: 2, width: "25%" },
                    { targets: 3, width: "15%", className: "text-center", type: "num" },
                    { targets: 4, width: "17%", className: "text-center", orderable: false, searchable: false }
                ],
                order: [[2, 'asc'], [1, 'asc']],
                pageLength: 10,
                lengthMenu: [[5, 10, 25, 50, -1], [5, 10, 25, 50, "Todos"]],
            ...
            initializeDataTable('#tablaContenidosCE', {
                columnDefs: [
                    { targets: 0, width: "6%", className: "text-center" },
                    { targets: 1, width: "25%" },
                    { targets: 2, width: "12%", className: "text-center", type: "num" },
                    { targets: 3, width: "15%" },
                    { targets: 4, width: "15%" },
                    { targets: 5, width: "12%", className: "text-center", type: "date" },
                    { targets: 6, width: "15%", className: "text-center", orderable: false, searchable: false }
                ],
                order: [[5, 'desc']],
                pageLength: 10,
                lengthMenu: [[5, 10, 25, 50, -1], [5, 10, 25, 50, "Todos"]],
        ...
        // Función para inicializar DataTable con configuración personalizada
        ...
        function eliminarTemaCE(id, nombre) {
            if (confirm('¿Estás seguro de eliminar el tema CE "' + nombre + '"? \n⚠️ ADVERTENCIA: Esto también eliminará todos los subtemas y contenidos asociados. Esta acción no se puede deshacer.')) {
        ...
        function eliminarSubtemaCE(id, nombre) {
            if (confirm('¿Estás seguro de eliminar el subtema CE "' + nombre + '"? \n⚠️ ADVERTENCIA: Esto también eliminará todos los contenidos asociados. Esta acción no se puede deshacer.')) {
                // Crear formulario temporal para envío DELETE
                const form = document.createElement('form');
                form.method = 'POST';
        ...
        function verTablaContenidoCE(id) {
            ...
            fetch(`{{ url('/sigem/admin/consultas/contenidos') }}/${id}`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                            ...
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    <h4 class="text-primary mb-2">
                                        <i class="bi bi-table me-2"></i>${contenidos.titulo_tabla || 'Tabla sin título'}
                                    </h4>
                                    <div class="d-flex flex-wrap gap-3">
                                        <span class="badge bg-primary fs-6">
                                            <i class="bi bi-bookmark-fill me-1"></i>
                                            ${contenidos.subtema?.tema?.tema || 'Sin tema'}
                                        </span>
                                        <span class="badge bg-success fs-6">
                                            <i class="bi bi-bookmarks-fill me-1"></i>
                                            ${contenidos.subtema?.ce_subtema || 'Sin subtema'}
                                        </span>
                                        <span class="badge bg-info fs-6">
                                            <i class="bi bi-grid-3x3 me-1"></i>
                                            ${contenidos.tabla_filas}×${contenidos.tabla_columnas}
                                        </span>
                                    </div>
                                </div>
                                <div class="col-md-4 text-end">
                                    <small class="text-muted">
                                        ...
                                        ${contenidos.created_at ? new Date(contenidos.created_at).toLocaleDateString('es-MX', {
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric',
                                            hour: '2-digit',
                    ...
                    if (contenidos.pie_tabla) {
                        htmlRespuesta += `
                            <div class="mt-3">
                                <small class="text-muted">
                                    <i class="bi bi-info-circle me-1"></i>
                                    <em>${contenidos.pie_tabla}</em>
                                </small>
                            </div>
                        `;
                    }
        ...
        function editarContenidoCE(id) {
            ...
            fetch(`{{ url('/sigem/admin/consultas/contenidos') }}/${id}`)
                ...
                .then(data => {
                    ...
                    if (contenidos.subtema && contenido.subtema.tema) {
                        ...
                        setTimeout(() => {
                            ...
                            const subtemaOption = subtemaSelect.querySelector(`option[value="${contenidos.ce_subtema_id}"]`);
                            if (subtemaOption) {
