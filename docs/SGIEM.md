...
5. **Auditoría del dataset por sesión**: apertura → snapshot en caché (8 h, **sin `headers`/`labels`** — árboles derivados que el modal de diff no consume); cierre → compara firma + mapa de celdas de **todas** las secciones y registra solo si cambió; cierre diferido vía middleware `CerrarSesionAuditoriaDataset` (grupo web; `cambios` ya no lo re-invoca). Los listados del módulo Cambios usan `limit(200)` por fuente y `select` liviano para `auditoria_datasets` (sin columnas JSON; flag `tiene_payload`).
...
8. **Dashboard**: métricas de visita (13 KPIs) sobre `pub_visita`/`pub_visitante`; la tabla de últimas visitas usa `limit(100)` (nunca `get()` sin límite sobre logs que crecen infinito).
...
- **No precargar `datos`/`categorias` en listados** (`Cuadro::obtenerTodos`/`obtenerPorId` solo precargan `subtema.tema`): el index de cuadros revientó el `memory_limit` de 128 MB del hosting al cargar todos los `cuadro_datos` del sistema (las vistas solo necesitan counts — index usa `withCount('categorias')` → `$cuadro->categorias_count`). Las operaciones del dataset siempre re-consultan por relación con filtros; acceder a las colecciones hace lazy-load on-demand. Extensión 2026-09-18 (§10 del 06): **nunca `get()` sin límite sobre logs que crecen infinito** (`pub_visita`, auditorías) — dashboards con selector «Últimas 100/250/500» (`?visitas`), módulo Cambios con `limit(200)` por fuente y `select` liviano; **la creación masiva de `cuadro_datos` usa `insertarDatosLote`** (batch chunks de 500, timestamps manuales); listados CRUD con `withCount` en vez de counts por fila; listado CE sin `tabla_datos`; export/documento con límite defensivo de celdas.
